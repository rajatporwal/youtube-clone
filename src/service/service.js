...
export const getVideosList = ({ query, pageToken, pageSize }) => {
    return new Promise((resolve, reject) => {
        ...
        fetch(`${YT_BASE_URL}${new URLSearchParams(requestObject)}`)
            .then((response) => {
                if (!response.ok) {
                    // If the response is not ok (e.g., 400 Bad Request), reject manually
                    return response.json().then((err) => {
                        reject({ status: response.status, message: err.error?.message || "API error" });
                    });
                }
